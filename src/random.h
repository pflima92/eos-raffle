...
// Code original from https://github.com/EOSBlox/random
...
namespace eosblox
{
    ...
    public:
        ...
        {
            eosio::check(min < max, "min cannot be gt than max");
            if (min >= max)
            {
                return min;
            }
        ...
        auto nextSample(const Container &population)
        {
            ...
            eosio::check(size > 0, "size cannot be gt than zero");
            if (size <= 0)
            {
        ...
        {
            eosio::check(n > 0, "n cannot be lt than 0");

            const auto size = population.size();
            eosio::check(size > 0, "size cannot be lower than zero");
